...
let Participante = (_dec = NearBindgen({}), _dec(_class = class Participante {
  //Inicializamos el objeto
  constructor(nombre, cuenta, edad) {
    this.nombre = nombre;
    this.cuenta = cuenta;
    this.edad = edad;
    this.certificado = false;
  }
}) || _class); //Creamos la clase principal del contrato
let StarterContract = (_dec2 = NearBindgen({}), _dec3 = call({
  payableFunction: true
}), _dec4 = view(), _dec5 = view(), _dec6 = call({}), _dec2(_class2 = (_class3 = class StarterContract {
  //Creamos una colección para almacenar información en nuestro contrato.
  participantes = new UnorderedMap('p');
  ...
  set_participante({
    ...
    log("Registro creado exitosamente.");
  }
  ...
  set_certificado({
    ...
    assert(signerAccountId() == "aklassen.testnet", "No tienes permisos para ejecutar este comando.");
    ...
    if (participante && participante.certificado == false) {
      ...
      const promise = promiseBatchCreate(cuenta);
      promiseBatchActionTransfer(promise, ONE_NEAR * BigInt(5));

      //Y guardamos los cambios hechos al participante
      this.participantes.set(cuenta, participante);
      log("Participante certificado. El participante ha recibido su recompensa de 5 NEAR.");
      ...
      //regresamos false.
      log("El participante ya se encontraba certificado.");
      return false;
    } else {
      log("Participante no encontrado.");
      return false;
    }
  }
}, (_applyDecoratedDescriptor(_class3.prototype, "set_participante", [_dec3], Object.getOwnPropertyDescriptor(_class3.prototype, "set_participante"), _class3.prototype), _applyDecoratedDescriptor(_class3.prototype, "get_participante", [_dec4], Object.getOwnPropertyDescriptor(_class3.prototype, "get_participante"), _class3.prototype), _applyDecoratedDescriptor(_class3.prototype, "get_participantes", [_dec5], Object.getOwnPropertyDescriptor(_class3.prototype, "get_participantes"), _class3.prototype), _applyDecoratedDescriptor(_class3.prototype, "set_certificado", [_dec6], Object.getOwnPropertyDescriptor(_class3.prototype, "set_certificado"), _class3.prototype)), _class3)) || _class2);
function set_certificado() {
  const _state = StarterContract._getState();
  if (!_state && StarterContract._requireInit()) {
    throw new Error("Contract must be initialized");
  }
  const _contract = StarterContract._create();
  if (_state) {
    StarterContract._reconstruct(_contract, _state);
  }
  const _args = StarterContract._getArgs();
  const _result = _contract.set_certificado(_args);
  StarterContract._saveToStorage(_contract);
  if (_result !== undefined) if (_result && _result.constructor && _result.constructor.name === "NearPromise") _result.onReturn();else env.value_return(StarterContract._serialize(_result, true));
}
function get_participantes() {
  ...
  const _args = StarterContract._getArgs();
  const _result = _contract.get_participantes(_args);
  if (_result !== undefined) if (_result && _result.constructor && _result.constructor.name === "NearPromise") _result.onReturn();else env.value_return(StarterContract._serialize(_result, true));
}
function get_participante() {
  ...
  const _args = StarterContract._getArgs();
  const _result = _contract.get_participante(_args);
  if (_result !== undefined) if (_result && _result.constructor && _result.constructor.name === "NearPromise") _result.onReturn();else env.value_return(StarterContract._serialize(_result, true));
}
function set_participante() {
  ...
  const _args = StarterContract._getArgs();
  const _result = _contract.set_participante(_args);
  StarterContract._saveToStorage(_contract);
  if (_result !== undefined) if (_result && _result.constructor && _result.constructor.name === "NearPromise") _result.onReturn();else env.value_return(StarterContract._serialize(_result, true));
}

export { get_participante, get_participantes, set_certificado, set_participante };
